...
def main():
    ...
    som_size = 64
    mmr_som = Som(som_size, 6, low=0, high=1, round_values=True)
    test_percentage = 0.2
    test_start_index = np.floor(tot_epoch*(1-test_percentage))
    my_learning_rate = ExponentialDecay(
        starter_value=0.5,
        decay_step=int(test_start_index/10),
        decay_rate=0.9,
        staircase=True
    )
    ...
    for epoch in range(1, tot_epoch):
        ...
        # Normalize data
        m_t_coord = [x/16 for x in m_t_coord]
        s_t_coord = [x/64 for x in s_t_coord]
        s_t1_coord = [x/64 for x in s_t1_coord]
